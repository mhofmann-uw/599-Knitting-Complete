    ...
    for row in range(0, height):
        ...
        if row % 2 == 0:
            ...
            for n in range(width - 1, -2 + height - row, -1):  # off by one?
                needle = Needle(True, n)
                knits[needle] = Instruction_Parameters(needle, involved_loop=-1, carrier=c1)
            _add_carriage_pass(Carriage_Pass(Instruction_Type.Knit, even_dir, knits, machine_state), carriage_passes,
                               instructions)
        else:
            # front odd_dir
            for n in range(height - row - 1, width):
                needle = Needle(True, n)
                knits[needle] = Instruction_Parameters(needle, involved_loop=-1, carrier=c1)
            _add_carriage_pass(Carriage_Pass(Instruction_Type.Knit, odd_dir, knits, machine_state), carriage_passes,
                               instructions)
    ...
    if bends:
        for i in range(0, len(bends)):
            ...
            if bend_dir is Bend_Direction.Left:
                ...
                print("shifted")
                iso_bend_shifted_helper(width, cur_bend.height, c1, machine_state, carriage_passes, instructions,
                                        bend_dir)
            else:
                raise AttributeError
